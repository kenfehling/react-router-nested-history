...
var Draggablee = require("react-draggable");
var Draggable = Draggablee; // Hide TypeScript errors
var noop = function () { };
...
var DumbHistoryWindow = /** @class */ (function (_super) {
    ...
    DumbHistoryWindow.prototype.onDragEnd = function (event, data) {
    ...
    DumbHistoryWindow.prototype.render = function () {
        ...
        if (storedVisible) {
            ...
            var w = (React.createElement("div", __assign({}, divProps, { ref: function (el) { return _this.calculateDimensions(el); }, className: this.getClassName(), onMouseDown: drag ? noop : this.onMouseDown.bind(this), style: __assign(__assign({}, style), { zIndex: zIndex, position: 'absolute', x: !drag ? x : undefined, y: !drag ? y : undefined }) }), getChildren()));
            var hasDefaultPosition = rememberPosition || this.state.width !== 0;
            if (drag && hasDefaultPosition) {
                return (React.createElement(Draggable, __assign({ grid: [1, 1] }, draggableProps, { onStop: this.onDragEnd.bind(this), onMouseDown: this.onMouseDown.bind(this), position: rememberPosition && x && y ? { x: x, y: y } : undefined, defaultPosition: hasDefaultPosition && x && y ? { x: x, y: y } : undefined }), w));
            }
            else {
                return w;
            }
