...
var DumbHistoryWindow = (function (_super) {
    ...
    DumbHistoryWindow.prototype.render = function () {
        ...
        var _a = R.omit([
            ...
            'storedPosition',
            'storeSubscription'
        ], this.props), children = _a.children, _b = _a.style, style = _b === void 0 ? {} : _b, zIndex = _a.zIndex, storedVisible = _a.storedVisible, switchTo = _a.switchTo, open = _a.open, close = _a.close, draggable = _a.draggable, _c = _a.draggableProps, draggableProps = _c === void 0 ? {} : _c, rememberPosition = _a.rememberPosition, divProps = __rest(_a, ["children", "style", "zIndex", "storedVisible", "switchTo", "open", "close", "draggable", "draggableProps", "rememberPosition"]);
        var drag = !!draggable && !!storedVisible;
        var w = (React.createElement("div", __assign({}, divProps, { ref: drag ? function (el) { return _this.calculateDimensions(el); } : noop, className: this.getClassName(), onMouseDown: drag ? noop : this.onMouseDown.bind(this), style: __assign({}, style, { zIndex: zIndex, position: 'absolute', display: storedVisible ? 'block' : 'none' }) }), children instanceof Function ? children({ switchTo: switchTo, open: open, close: close }) : children));
        if (drag) {
            var x = this.calculateX();
            var y = this.calculateY();
            return (React.createElement(Draggable, __assign({}, draggableProps, { onStop: this.onDrag.bind(this), onMouseDown: this.onMouseDown.bind(this), position: rememberPosition ? { x: x, y: y } : undefined, defaultPosition: rememberPosition ? undefined : { x: x, y: y } }), w));
        }
        else {
            return w;
        }
