...
describe('main', () => {
  ...
  describe('memoryHistory tests', () => {
    ...
    const run = (actions:Object[]) : Promise =>
        runSteps(util.createStepsSinceLastUpdate([...createContainers, ...actions], new Date(1)))
    ...
    it.only('reacts to being on the zero page during switch', async () => {
      await run([
        {type: LOAD_FROM_URL, time: new Date(0), data: {url: '/a'}},
        {type: BACK, time: new Date(0), data: {n: 1}},
        {type: SWITCH_TO_CONTAINER, time: new Date(0), data: {groupIndex: 0, containerIndex: 1}}
